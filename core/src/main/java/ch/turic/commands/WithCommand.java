...
public class WithCommand extends AbstractCommand {
    ...
    public Object _execute(final LocalContext context) throws ExecutionException {
        ...
        final var objects = new ArrayList<HasFields>();
        try {
    ...
    /**
     * Wraps a context by processing an object in a {@code with} statement and
     * binding its entry result.
     * <p>
     * This method evaluates the result of an expression in a {@code with}
     * statement. If the result is a
     * {@link LngObject} and an alias is provided, it attempts to invoke the
     * {@code entry} method on the object.
     * The return value of this method must also be a {@link LngObject}. The result
     * is then bound to the alias
     * in a wrapped version of the context.
     * <p>
     * If no alias is provided, the context is updated with the context of the
     * {@link LngObject} directly.
     * <p>
     * Any violation—such as a missing or invalid {@code entry} method when 'as'
     * alias is defined,
     ...
     * @param pair    the {@link WithAnalyzer.WithPair} representing the expression
     *                and optional alias
     * @param obj     the evaluated result of the expression in the {@code with}
     *                clause
     * @param objects a list to which processed {@link LngObject}s with aliases are
     *                added
     * @param ctx     the current context to be wrapped or updated
     * @return a new {@link LocalContext} instance that includes any alias bindings
     *         or updated context information
     * @throws ExecutionException if the object is not a {@link LngObject}, has no
     *                            valid {@code entry} method,
    ...
    /**
     * Either returns {@code null} to indicate suppressed failure, or throws the
     * given exception.
     * <p>
     * If any {@link RuntimeException}s are present in {@code closeExceptions}, they
     * are added as
     * suppressed exceptions to the provided {@link ExecutionException}.
     * <p>
     * If {@code suppressExceptions} is {@code true}, the method returns
     * {@code null}, indicating that
     * the execution should continue without signaling an error. Otherwise, it
     * throws the enriched
     * {@link ExecutionException}.
     *
     * @param closeExceptions    a list of {@link RuntimeException}s thrown during
     *                           resource cleanup
     * @param exception          the {@link ExecutionException} to throw if
    ...
    /**
     * Throws exceptions that occurred during resource closure if no primary
     * execution exception is present.
     * <p>
     * If {@code exception} is {@code null} and {@code closeExceptions} contains one
     * or more exceptions,
     * the method throws an appropriate exception:
     * <ul>
     * <li>If exactly one exception is present, it is rethrown directly.</li>
     * <li>If multiple exceptions are present, a new {@link ExecutionException} is
     * thrown with
     * all of them added as suppressed exceptions.</li>
     * </ul>
     * <p>
     * If {@code exception} is not {@code null}, this method does nothing.
     * In that case the exceptions will be thrown later by the upper layers that
     * handle the non-null exception.
     *
     * @param exception       the primary {@link ExecutionException}, or
     *                        {@code null} if none occurred
     * @param closeExceptions a list of {@link RuntimeException}s thrown during
     *                        resource closure
     * @throws ExecutionException if multiple closure exceptions occurred and no
     *                            primary exception is present
     * @throws RuntimeException   if a single closure exception occurred and no
     *                            primary exception is present
     */
    ...
    /**
     * Invokes the {@code exit} methods on a list of {@link LngObject}s in reverse
     * order.
     * <p>
     * Each object is inspected for a field named {@code METHOD_NAME_EXIT}. If such
     * a field exists and is a
     * {@link Closure}, it is invoked with a single parameter: a wrapped
     * {@link LngException} if an
     * {@link ExecutionException} is provided, or {@code null} otherwise.
     * <p>
     * If any closure returns {@code true}, it indicates that the exception should
     * be suppressed;
     * this intent is reflected by updating the {@code suppressExceptions} flag.
     * <p>
     * Any exceptions thrown during the retrieval or invocation of the {@code exit}
     * method are wrapped
     * in a {@link RuntimeException} and collected into the {@code closeExceptions}
     * list.
     *
     * @param context            the current execution {@link LocalContext}, used
     *                           for building exceptions and calling closures
     * @param exception          the {@link ExecutionException} that caused the
     *                           termination, or {@code null}
     * @param lngObjects         a list of {@link LngObject}s whose {@code exit}
     *                           methods are to be called
     * @param suppressExceptions an {@link AtomicBoolean} flag indicating whether
     *                           exceptions should be suppressed;
     *                           updated based on the return values of the
     *                           {@code exit} closures
     * @param closeExceptions    a list to collect any {@link RuntimeException}s
     *                           thrown during method execution
     */
    private static void callExitMethods(final LocalContext context,
            final ExecutionException exception,
            final ArrayList<HasFields> lngObjects,
        ...
        for (final var lngObject : lngObjects.reversed()) {
            try {
                ...
                if (entry instanceof Closure closure) {
                    final var exitValue = Cast
                            .toBoolean(closure.callAsMethod(context, lngObject, METHOD_NAME_EXIT, param));
