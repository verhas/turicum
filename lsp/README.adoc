...
A built-in rule table determines the number of spaces between tokens.
...
These built-in formatting rules are defined in the project resources.
...
* `IDENTIFIER`
* `RESERVED` will match non-keyword symbols
* `STRING`
* `INTEGER`
* `FLOAT`
* `COMMENT`
* `CHARACTER`^*^
* `SPACES`^*^
* `KEYWORD`^*^ will match keywords

for the token types.

.Note on the types ^*^ `CHARACTER`, `SPACES`, and `KEYWORD`
[NOTE]
====
These three are special token types and will never appear in the tokenized version of a correct Turicum program.
...
`SPACES` is used internally by the Language Server Processor formatter, and it skips them.
Such a token rule will not match any token pairs.

`KEYWORD` will match any token of the type `RESERVED` when it is a keyword, like `with`, `if`, and so on.
`RESERVED`, on the other hand, will match a token only when it is `RESERVED` and not a keyword, like `===`, `:`, and so on.
The lexical analyser sets the type of a token representing a keyword or a symbol to `RESERVED` and it does not uses the type `KEYWORD`.
It is used only by the formatter to ease formatting rule maintenance.
====
...
Parsing stops when a rule matches the two tokens currently being processed.
...
The default rule files used when there is no local formatting rules defined is the following:
...
[
    ...
    between( FLOAT, ".", 0),
    between( STRING, ".", 0),
    between( INTEGER, KEYWORD, 1),
    between( FLOAT, KEYWORD, 1),
    between( STRING, KEYWORD, 1),
    between( KEYWORD, INTEGER, 1),
    between( KEYWORD, KEYWORD, 1),
    between( KEYWORD, FLOAT, 1),
    between( KEYWORD, STRING, 1),
    between( KEYWORD, IDENTIFIER, 1),
    between( INTEGER, RESERVED, 1),
    between( FLOAT, RESERVED, 1),
    between( STRING, RESERVED, 1),
    between( RESERVED, INTEGER, 1),
    between( RESERVED, FLOAT, 1),
    between( RESERVED, STRING, 1),
    between( KEYWORD, "[", 0),
    between("]", ",", 0),
    between(":", "[", 1),
    between(")", ",", 0),
...
If there is an error in the file, some rules are not correctly formatted, it will write an error to the error output.
