...
In the first chapter, we describe how to use the debugger.
Later chapters describe how the debugger is implemented.

== Using the Debugger

You can use the REPL command line version of Turicum to debug a Turicum program.
To start the REPL functionality, you type at the command line:
...
instead of `turi.debug`.
It will define the same functions and also abbreviation aliases, like `si` for `step_into`, `so` for `step_over`, and so on.
