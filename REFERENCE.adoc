...
* **Flexible**
+
The language's syntax is extensible.
You can write a preprocessor to hook into the processing chain between lexical and syntax analysis.
The preprocessor can be written in Turicum itself, and it works on the lexical token list.

* **Multithreaded**
+
The interpreter is built on Java 21 and supports Virtual Threads using native language constructs.
...
The Maven extension is automatically downloaded and started if you have an `extensions.xml` file in your project's `.mvn` directory.
...
If you want to embed the application into a Java or Kotlin application, use the following dependency:
...
For an example, see the `cli/pom.turi` project definition.
It is the command line version of Turicum that depends on the core library.

An embedder controls how much a Turicum program is allowed to run through the `LocalContext` constructor: a step limit bounds total execution, and an optional bounded grace allowance lets `finally`/`with` cleanup code run for a little while after that limit (or an external abort) fires, without being able to run forever — see <<cleanup-after-halt>>.
Richer embedding and sandboxing APIs, including limiting a program's access to the JDK and system resources, are under active development.
...
Symbolic constants are the following:
...
You can define a method with the name `pass:[`+`]`.
====
...
Multiline string literals are enclosed in triple quotes `pass:["""]`.
...
You can also omit the `(` and `)` characters if the expression is a single variable.
...
The values can be any Turicum expression.
...
However, it is called each time the field is accessed.
Lazy JSON objects are pinned, meaning it is not possible to change any of their fields.
...
If you want to append a list to a list instead of concatenating, you have to enclose it in another list.
...
When a method is invoked, the special variables `this` and `cls` are set the same way as if the call were performed on the original object or class.
...
Other operators, like `+`, `-`, `*` and so on, which are defined for lists will inherit the attachment of the left-hand side operand.
...
[source]
----
hello {this: {...}, Kukk: 2, cls: class B}, class B  [1, 2, 3]
2
Now with h
hello none, class B  [1, 2, 3]
none
hello {this: {...}, Kukk: 2, cls: class B}, class B  [2, 3]
...
NOTE: You can also use a keyword as a field name or method name, either enclosed between `pass:[`]` or without.
Turicum allows you to use a keyword like `as` without the backticks around it `pass:[`as`]` when they stand right after `.` or `?.`.
...
They will automatically become an `+{}+` classless object.

Accessing a field on the right-hand side of an assignment, or in any expression, is more restrictive.
You can still access a nonexistent field, and the value will be `none`, but you cannot access a field on `none`.
To overcome this, you can use the `or` operator as in the example, or you can use the `?.` field access.
...
On the other hand, there is no `?.` equivalent for accessing a field through `["xxx"]`.
...
* `or` results in the left side expression if it is defined, and the right side if the left side value is `none` (undefined).
The operator does a shortcut evaluation.
...
* `..` range operator. `start..end` creates a range to index a list.
...
+
The operator works the conventional way on integers.
+
...
* `!==`, `===`, `==`, `!=`, `in` are __not-the-same__, __the-same__, __equal__, __not-equal__, and __part-of__ operators.
...
The operator `!==` tests that the two objects are not the same.
This operator is the negated version of `===`.
...
// checks member not sublist
println [ 1, 2] in [ 1, 2, 3], " [1, 2] is not in [ 1, 2, 3]"
println [ 1, 2] in [ 1,[ 1, 2], 3], " [1, 2] is in [ 1, [1, 2], 3]"
...
false [1, 2] is not in [ 1, 2, 3]
true [1, 2] is in [ 1, [1, 2], 3]
...
+
The unary `+` operator does not perform any checks; it operates on all objects and returns the object itself.
+
** The binary `+` operator
+
*** concatenates strings (second argument will be converted to string if not string already),
*** concatenates unevaluated (quoted, as `'$"${a}" +'$"${a}"`) strings
*** concatenates unevaluated (quoted, as `'$"${a}" +"a"`) strings with evaluated string
*** concatenates quoted blocks and quotes list expressions,
*** concatenates lists
*** merges objects, and
*** eventually, works on numerical values.
+
** The binary `-` works
+
*** on strings, removing all occurrences of the second argument from the first,
*** on lists, removing the elements of the second argument from the first, and
*** eventually, it also works on numerical values.

* `pass:[*]`, `/`, `%` binary multiply, divide, and remainder operators.
The `pass:[*]` and `%` are also used in the case of strings and lists.
A string multiplied by a number results in the string concatenated to itself that many times.
...
Many languages use `^` to denote this operator; however, in Turicum, that character is for bitwise XOR.
+
...
The expression analyzer recognizes and allows you to use pre-increment and pre-decrement operators inside an expression.
It is easy for the parser -- and similarly for the programmer -- to recognize that a `++` or `--` introduces a pre-increment/pre-decrement assignment.
...
The next two examples implement the same functionality for the `--` operator.
...
Operators work for numerical types, string types, as usual, and they can also work on any object that defines them.
The name of the method implementing the operator should be the operator itself.
There is no special name mapping, like Python's dunder methods (e.g., `pass:[__add__]`).
...
When an operation is used as a prefix unary operator, the value of the other will be `none`, and `this` will hold the value on the right of the operator.

Only `+`, `-`, `~`, `!`, `..`, `?`, and `pass:[']` can be used as unary operators.
...
* The unary operator `?` returns the original value without any modification or `none` if the calculation of the argument resulted in an error.
  The original value can also be `none`, that way you cannot distinguish from the value if it is a genuine `none` or the result of an error.
* The unary `pass:[']` quotes the right-hand side, and has the same effect as the `quote` or `thunk` built-in functions.
  When you `'{}` you get an empty block.
  On the other hand, `thunk({})` will be an empty **object**, resulting in `{}` when executed.
  If you execute `('{}).execute` you will execute an empty **block**.
  The result in this case is `none`.
...
NOTE: The character `?` can have a different meaning based on the context.
...
class A {
  fn `==`(other) : bool {
    return false when cls != other.cls
    a == other.a
  }
}
...
The order of operator execution can be altered using parentheses.
...
Because every command is a value and every expression is a command, you can use `()` to group commands to be executed in the same context as the surrounding code.
...
Scopes are created at runtime, like everything in Turicum.
...
class A {
  fn z(){
    ...
    try{
       ...
       // you could write 'let/mut glb', then it is a local variable, or
       // 'global glb', then it is a global variable
       glb = 5; // this will create an error
    }catch(e1){
        println("exception 1:",e1)
    }
    try{
       let h = glb // after the try it is a new scope
...
exception 2:Variable cannot be local, it is already used as non-local 'glb'
...
The methods -- functions defined inside a class -- wrap the context of the class.
Whenever a method is called its own context is created and the "local variables" `cls` and `this` are created.
Unless the method is a constructor (named `init`), these variables are pinned and cannot be changed.
...
The closure wraps this context and keeps the variable alive as long as the closure exists, and it also modifies the content of the variable.
...
When a closure argument list defines types and alternatives for the types, how does the parser know whether the `|` is separating different types or closing the closure argument list?
...
This analysis is heuristic and can cause issues.
To avoid those:
...
This way, macros can decide which arguments to evaluate and in what order, or how many times.
...
print c(true,"yes, it is true",{ println("this will never be printed") })
...
This ensures the same context and variable visibility as calling a function or closure.
The caller does not need to know whether what it calls is implemented as a function, closure, or macro.

=== Variable Declaration

On first assignment, the code must use the keyword `let` or `mut` to define the variable.
...
Although `mut` is not strictly declarative, you can declare new variables in a `mut` mapping as in the example above.
...
Although there is no limit on the nesting depth of mapping and destructuring, it is usually not good practice to create excessively deep structures.


==== Updating a Variable

Mutable variables can receive a new value at any time.
...
which simply looks like `a = 5*3` in most cases.
...
In the case of array elements, pinning is not possible for the individual elements.
...
[source]
----
Assigned value changed while calculating new value z[z]
12 already changed
----
...
Doing so is essentially the same as assigning a new value to the variable altogether:
...
When you change a part of a string, Turicum creates a new string and then updates the underlying variable to hold this new string.
...
In this case, the previously defined variable will be updated.
...
There are some restrictions on this kind of variable update:
...
The list contains comma-separated elements, which can be
...
It is an error to use the decoration on a variable that is not an object or list.
...
Once upon a midnight dreary, Edgar
God in his wisdom made the fly, Ogden
...
The value of the command is the string sent to the output.
...
If the expression evaluates to `true`, the first command or block is executed.
If the expression evaluates to `false`, the second command or block following the `else` keyword is executed.
The `else` keyword and its command or block are optional.
...
The `:` following the `else` keyword is optional.
Its use can emphasize the structure and may help Python lovers.

The conditional expression can be enclosed in parentheses but does not need to be.
...
The condition, the commands following it, and the optional one after the `else` keyword are evaluated and executed in the context of the `if` statement.

If, however, the "then" or "else" commands are block commands, they execute in a new context that wraps the actual context.

=== Conditional expression

`if` statements can be used inside expressions.
When you use `{}` or `()` parentheses, you can use any statement in an expression, but `if` statements do not need parentheses.
An `if` statement can be placed on "top-level" in an expression and can occupy a place that is for a whole expression inside another one.

.source of snippet `if_expression_1`

[source]
----
print 5 + if 5 > 4 : 1 else 2

----

.output of snippet `if_expression_1`
[source]
----
6
----
...
The `if` statement in this example serves as the second argument for the `+` operator.

The keyword can also be used following the syntax

    expression_2 if condition else expression_2

This structure resembles how Python uses conditional expressions.
The following example gives this variant for the previous calculation:

.source of snippet `if_expression_2`

[source]
----
print 5 + 1 if 5 > 4 else 2

----

.output of snippet `if_expression_2`
[source]
----
6
----
...
Note, however, that you may need parentheses when using this form:

.source of snippet `if_expression_3`

[source]
----
println 5 + 1 if 5 < 4 else 2
println (5 + 1) if 5 < 4 else 2
println 5 + (1 if 5 < 4 else 2)

----

.output of snippet `if_expression_3`
[source]
----
7
2
...
The `if` in the expression stands on the highest level in precedence.
Since the `if` "operator" and its precedence is not so obvious as the usual numerical operators, it is recommended to use parentheses for readability.
...
If the loop is skipped, the value is either `none` or an empty list.
...
The `while` looping construct is a complex structure.
It can contain up to five command blocks and two conditional expressions executed at different points.
The full syntax of the command is the following:
...
image::pictures/while_loop.png[]

It

* executes the block following the `loop`, then

* iterates
  ** 'while' the condition after the `while` is true, or
  ** 'until' the condition after the `until` becomes true, or
  ** until a `break` or `return` command is executed in the loop.

If the `while` condition was false already at the start and the iteration DID NOT start, the block after the `otherwise` is executed.

If the iteration started and finished without executing a `break` or a `return` command, the block after the `done` is executed.

Finally, the block following the `finally` keyword is executed.
This block is executed even if a `break` or `return` command was executed in the loop.
The special, pinned variable `it` will hold the loop's value in these blocks.
This variable is not defined when the block `otherwise` is executed.

The loop is executed in two contexts.
The outer context wraps the surrounding context.
The inner context wraps the outer.
The loop body is executed in the inner context.
A new inner context is created for each execution of the loop.
The `while` condition and the other blocks are executed in the outer context.
The `until` condition is executed in the inner context.
...
done { println "loop is done" }
otherwise { println "Loop did not even start" }
finally { println "We are done here" }
try: print i catch err: println err
...
The value of the loop is the result of the last executed command in the loop body.
...
The value from the `done`, and `finally`  blocks do not affect the loop's value.

.source of snippet `while_result_with_done`

[source]
----
println {
    mut k = 0
    while k < 10 : k++ done: 666;
}

----

.output of snippet `while_result_with_done`
[source]
----
666
...
.source of snippet `while_result_with_finally`

[source]
----
println {
    mut k = 0
    while k < 10 : k++ finally: 666;
}

----

.output of snippet `while_result_with_finally`
[source]
----
9
...
The value of the loop is the `otherwise` block's value if that was executed.

.source of snippet `while_result_with_otherwise`

[source]
----
println {
    mut k = 0
    while false : k++ done: k otherwise: 42;
}

----

.output of snippet `while_result_with_otherwise`
[source]
----
42
...
The loop can also be used to produce a list.
...
[source]
----
println { loop: mut k = 0 while k < 5 list: k++ }
...
For list resulting loops, when the `done` and/or `finally` blocks are executed, the special variable `it` will contain the resulting list.
The variable as well as the list itself are pinned, so you cannot add or remove elements from the list or change the elements.

.source of snippet `while_with_done_list1`
...
let k = {
    ...
    while i <= 10 list{
      i++;
          ...
          // cannot do that the variable is pinned
          mut e;
          try: it = [3,2,1] catch e1: println e1
          // cannot do that the list itself is pinned
          try: it[0] = 100 catch e2: println e2
      }
    }
print k;
...
.output of snippet `while_with_done_list1`
[source]
----
Variable 'it' is pinned.
Cannot change a pinned list.
...
.a note on the execution context of loops
[NOTE]
====
...
* The `loop` block, as well as
* the *head condition* following the `+while+` keyword are evaluated in the outer context.
...
The `loop` part clearly separates the loop from the surrounding code.
Variables declared here are used only in the loop and are shared across iterations.

.source of snippet `while_with_loop`

[source]
----
loop: mut i = 1
while i <= 10 list : i++ finally: println $"The last i after the loop is $i"
try: println "i does not exist outside of the loop" catch e: println e

----

.output of snippet `while_with_loop`
[source]
----
The last i after the loop is 11
...
The following example demonstrates that `otherwise` is not executed when the loop is executed at least once.

.source of snippet `while_until_not_otherwise`
...
while i <= 10 {
 ...
 otherwise: die "i is not less than 10"

----

.output of snippet `while_until_not_otherwise`
[source]
----
i is 1
i is 2
...
The next example shows that the `otherwise` is executed when the loop head condition is false from the very start.

.source of snippet `while_otherwise`

[source]
----
mut i = 20
while i <= 10 {
  die "i is 20, must not execute the loop."
  } otherwise: println "we are other, we are wise"

----

.output of snippet `while_otherwise`
[source]
----
we are other, we are wise

----



The behavior is the same even with an `until` exit condition, which is never evaluated in this case.

.source of snippet `while_until_otherwise`
...
.output of snippet `while_until_otherwise`
[source]
----
we are other, we are wise
...
The next example demonstrates that the `done` block is not executed if the loop is broken.

.source of snippet `while_until_with_done_broken`
...
while i <= 10 {
  println $"i is $i"
  break {
    println "we broke the loop"
    55
    } when i > 5
  i++;
  } until i > 10
  done {
      die "we broke the loop, we are not done"
  }

----

.output of snippet `while_until_with_done_broken`
[source]
----
i is 1
i is 2
...
.a note on Python loop `else:` constructs in Turicum
[NOTE]
====
Python has an `else:` construct that can follow a loop.
In Python, it is executed when the loop completes without being broken.
Turicum does not need to provide this extra block for the loops.

In Turicum, if you break a loop and want to execute some extra code, you can do it in the expression of the `break`.
The break command can provide an expression, and every command, or command block, is an expression in Turicum.
As you can see in the example above, this feature can be used for the same purpose as the `else:` branch for loops in Python.
====

The following two examples show the case when the `done` block is executed after the normal termination of the loop.

.source of snippet `while_with_done`
...
while i <= 10 {
  ...
  i++;
      ...
      println "we are done nice and neat, not broken"
  }

----

.output of snippet `while_with_done`
[source]
----
i is 1
i is 2
...
i is 9
i is 10
we are done nice and neat, not broken

----



It is also a normal termination of the loop when the loop is terminated by the `until` condition:

.source of snippet `while_until_with_done`
...
.output of snippet `while_until_with_done`
[source]
----
i is 1
i is 2
...
The syntactic difference is that the start, condition, and step parts do not need to be enclosed between `(` and `)`.
...
You can also define an index variable using the `with` keyword.
...
The index starts from zero and increments by one with each iteration.
...
That would violate the locality principle: do not rely on anything outside a context that can change, and even less modify anything outside the context, which in this case is a loop.
...
Continues the execution of the loop, starting the body again without executing the rest of the commands in the current iteration.

=== Returning, Breaking, Continuing, and Yielding

These commands belong to one family.
Each of them can pass a value, and each can have a condition following the expression.
The syntax is

[source]
----
break [expression] [when condition]
return [expression] [when condition]
yield expression [when condition]
----

The expression and the ``when condition`` parts are optional, except for the `yield`.
`yield` must have the expression.
When the conditional part is missing, the command is executed as if it were `when true`.

NOTE: Earlier versions permitted the use of the `if` keyword, but it became ambiguous when the `pass:[x if condition else y]` expression syntax was introduced.
...
When the `break` command is used in a list-producing loop and there is an expression following the `break` keyword, the value of this expression will be appended to the list as the last element.
...
In the first case, the statement executes within the loop, and the `break` statement aborts the loop's execution.
...
When the expression following the `break` keyword starts with the `if` keyword, it must be enclosed in `()` parentheses.
...
println {
    for i=0 ; i < 4 ; i++ {
      break if i == 2 : 3 else : 4 when i == 2;
      println i;
    }
}
...
The `continue` command can have an expression, but it is only allowed if the loop is list-producing.
...
You cannot use `continue` with a value in a loop that does not produce a list:
...
The typical example is when you execute a command with `async`.
...
The `with` command executes a block in the context of one or more objects, and it also functions as a resource manager similar to Python.
...
----
Creating resource
entry to ResourceManager {res: {this: {...}, cls: class Resource}, this: {...}, cls: class ResourceManager}
r resource is {this: {...}, cls: class Resource}
printf was called
executing with resource 1 2
exit fom ResourceManager {res: {this: {...}, cls: class Resource}, this: {...}, cls: class ResourceManager} with "mad dog" as exception
mad dog
closing resource
did not die
...
If the return value of the method `exit` is `true` (or anything that can be converted to `true`), then the exception is treated as caught and handled.
...
If there is more than one resource in a `with` statement, their `entry` methods are called from left to right and their `exit` methods from right to left.
...
* `exit` cannot suppress exceptions on its own
...
You can only create new fields in the rightmost object.
...
It is important to note that the `with` command can also be used with classes.
The `with` command can handle classes similar to objects and also as resource managers.
Using a class as a resource manager instead of an object is not recommended.
Remember: each resource manager can manage a single resource.

When you use a class in the `with` command instead of an object the class fields and methods will be invoked.

.source of snippet `withClass`
...
=== Mutual Exclusion: `mutex` and `sync`
...
is_held: true
is_locked: true
after unlock is_locked: false
caught: boom
released after die: true
...
The function `atomic()` creates an atomic cell holding a single value.
The value is reachable only through the synchronized operations of the cell; there is no way to access it without synchronization.
Any value can be stored, not only numbers.

The operations of the cell are:

* `get()` returns the current value.
It does not use a lock; it is safe and fast to call from any thread.
* `set(v)` replaces the value.
* `update(f)` atomically replaces the value with `f(value)` and returns the new value.
The function runs exactly once, while the cell is locked.
Do not perform blocking operations and do not use other atomic cells or mutexes inside it.
* `cas(expected, new)` sets the value to `new` if the current value equals `expected` and returns `true` if it did.
* `incr()`, `decr()`, and `add(n)` are numeric conveniences; they atomically modify the value and return the new value.
They report an error when the stored value or the argument is not a number.

Lists and objects stored in the cell are pinned: the cell holds and hands out immutable snapshots.
To change the value, build a new one and `set()` or `update()` it in.
This way, a value read from the cell can never be modified behind the cell's back.

.source of snippet `atomic_cell`

[source]
----
// The atomic() built-in creates a cell holding a single value. All mutating operations are
// synchronized on the cell, reads are lock-free, so threads can share the cell safely.
let a : atm = atomic(0)
...
a.set(100)
println "cas hit: " + a.cas(100, 5)
println "cas miss: " + a.cas(100, 7)
println "get: " + a.get()

// update atomically replaces the value with f(value) and returns the new value
println "update: " + a.update({|x| x * 2})

// any value can be stored; lists and objects become pinned (immutable) snapshots
let boxed = atomic([1, 2])
println "boxed: " + boxed.get()
let peeked = boxed.get()
try {
    peeked[0] = 9
} catch e {
    println "stored snapshot is pinned"
}
println a

----

.output of snippet `atomic_cell`
[source]
----
incr: 1
add: 10
...
The type of an atomic cell is `atm`, as in

[source]
----
let counter : atm = atomic(0)
----

Tasks started with `async` receive frozen copies of the surrounding variables; they cannot assign the variables of the parent.
An atomic cell is shared by reference: the frozen copy in the task refers to the same cell, so a task can `incr()` a counter or publish a result that the parent reads with `get()` without any further ceremony.
...
The return value of the `async` command is a `task`.

To wait for an asynchronously started command to finish, use the `await` command.
...
* `is_timeout` is `true` when the task was stopped due to timeout
...
* `close` closes the channel to the task.
...
* `has_next` will be `true` if there are objects sent by the task

* `next` gets the next value from the task

The command `async` has four parameters:

* `in` sets the size of the queue for objects to be sent to the task.
The default value is unlimited.
If the queue is limited and full, sending an object will block until a place is freed.

* `out` sets the size of the queue for the task to yield values back to the main thread.
...
If you do not limit the channel size, the generator will run ahead and fill the memory with prime numbers.
By limiting the channel size, you can balance the calculations.
The generator will stop when it has generated enough numbers and resume when some of the results are consumed.
...
fn is_prime(x:num):bool {
    return true when x == 2;
    return false when x % 2 == 0;
    for n = 3 ; n*n <= x ; n = n + 2 :
        return false when x % n == 0
    true
}
/**
...
Thread 1 generated 3
 prime read Thread 1 generated 5
3
Thread 1 generated 7
 prime read 5
 prime read Thread 1 generated 11
...
The task will stop after this many steps.
When a task exceeds the step limit the task `is_done()` and `is_err()` methods will return `true`.
The error message will indicate the step limit that was reached.
See <<cleanup-after-halt>> for what happens to `finally`/`with` cleanup code when the step limit is reached (or the task is aborted from outside).

.source of snippet `async_step_limit`

[source]
----
let z=async[ steps= 100] {
    loop{
        mut i= 0
        } while{
        print $"${i++} "
...
// sleep 100ms, should be enough to timeout
sleep 0.1

println "\nResults are true, true and false"
println $"  z is done ${z.is_done()}"
println $"  z is err ${z.is_err()}"
println $"  z is timeout ${z.is_timeout()}, it is step limit, not time"
println $"  ${z.get_err()}"

----

.output of snippet `async_step_limit`
[source]
----
...
Results are true, true and false
  z is done true
  z is err true
  z is timeout false, it is step limit, not time
  Step limit 100 reached

----



* `time` limits the execution time of the task; the unit is seconds.
The task will stop after the specified time.
When a task times out, the underlying thread stops and the task's `is_done()`, `is_err()`, and `is_timeout()` methods will return `true`.

These parameters can be specified after the `async` keyword between `[` and `]` characters.
When the command to be executed asynchronously starts with a `[` character, you either have to place `[]` in front of it,
or enclose the command between `(` and `)` or `+{+` and `+}+` to help the syntax analysis.

.source of snippet `async_timeout`

[source]
----
let z=async[ time= 1 / 20] {
    while{
...
println "All three results should be true"
println $"  z is done ${z.is_done()}"
println $"  z is err ${z.is_err()}"
println $"  z is timeout ${z.is_timeout()}"

----

.output of snippet `async_timeout`
[source]
----
All three results should be true
  z is done true
  z is err true
  z is timeout true
...
The command can also execute a list in parallel.
When the expression to execute asynchronously is a list literal, the command will execute the individual elements of the list in parallel.
In that case the result is a list of tasks instead of a task that results in a list.

[NOTE]
====
Do not enclose the list literal between `(` and `)` when using `async`.

* `[1, 2, 3]` is a list literal.
`async` will evaluate it as a parallel list.

* `([1, 2, 3])`, and `{[1, 2, 3]}` are block commands with a list literal in them.
`async` will evaluate the block as a single command.

The only way to create an async list is to provide the possibly empty options as in the next example.
====

The following example creates a list of `[1,2,3]` in the conventional way and also in parallel.
...
As you can see, in the first case the execution time is a bit above 300ms, which is three times the 0.1s wait in the function.
In the second case the functions wait the same time in parallel, and the execution time is just above the 100ms single wait time.
...
Scheduling and executing dependent tasks begins once all start cells have finished and the flow has reached its initial state.

If there are cells that have dependencies (and therefore are not start cells), but whose transitive closure of dependencies does not include any start cells, it is an error.
This is detected at compilation time and results in a syntax error.
...
It does not enter any symbol table and does not hide any object of the same name in the context hierarchy.
...
- `limit <maxTasks>`::
Optional.
A numeric expression that sets the maximum number of dependent cell executions allowed.
It is evaluated once, when the flow starts.
- `timeout <seconds>`::
Optional.
A numeric expression (can be fractional) specifying the maximum wall-clock time allowed for execution, in seconds.
It is evaluated once, when the flow starts.
...
The syntax analyzer cannot tell whether they start the limit or timeout definition or you want to name the flow "limit" or "timeout".
You cannot name the flow either of these.
====

The values of the clauses are parsed as expressions, the same way as the conditions of the `if` and `while` commands.
A bare identifier is a valid condition, and no `;` is needed between the clauses or before the body:

[source]
----
flow until finished limit 100 timeout 1.5 {
   ...
}
----

You can still write a `;` after each clause if you find it more readable.
Commands are not expressions in Turicum; if the condition needs one, enclose it in `{` and `}` as a block expression.
...
No two __start cell__s should update the same __cell state variable__.
...
The last check is that all dependency cycles contain at least one __start cell__.
If there is a dependency cycle that does not contain a __start cell__, that cycle would use a __cell state variable__ before it is initialized.

If any of the checks fail, the compiler reports a syntax error and the command is not created.
...
When a result is `fini`, the __cell state variable__ counter is updated even though the variable value is not; thus any block that was started earlier and not yet finished will not update the variable.

When the result is `non_mutat`, nothing is updated.
...
This context contains copies of the flow context variables, which are read-only.
...
When a cell variable that a block depends on changes (because the block uses it in some calculation), the block is scheduled for execution and started as soon as the JVM and the operating system allow.
Each cell block is executed in a new virtual thread.

It can happen that the same block executes in parallel in multiple instances.
This could lead to instability in the flow.
An earlier-started instance can run longer than a later one.
...
The __cell state variable__s are local to the `flow` command.
A flow does not care what variables are defined around it: a cell may freely have the same name as a variable of the surrounding scope or a global variable.
The cell is used inside the flow; the outer variable is neither read nor modified.
In particular, before a cell has computed its first value, the `until` condition and the other cells never fall back to a same-named outer variable.
Variables of the surrounding scope that do not collide with any cell name can be used freely in the cell expressions and in the `until` condition.

.source of snippet `flow_cell_locality`
...
.output of snippet `flow_cell_locality`
[source]
----
counted: 5
doubled: 2
...
==== Undefined Variables in the Exit Condition

The `until` condition is evaluated repeatedly, and during the start-up of the flow it may reference cells that have not computed their first value yet.
When the evaluation of the condition fails because a *cell* is undefined, the condition counts as `false` and the flow keeps running; the cell will get its value and the condition will be evaluated again.
When the evaluation fails because a name is undefined that is *not* a cell — for example, a typo — the flow stops and reports the error.
Any other error in the evaluation of the condition also stops the flow and reports the error.
...
Final x = Task limit has been reached in flow '#unnamed' command after 5 tasks.
...
The function becomes usable after the function definition is executed.
...
When it does not have a name, `(` and `)` must be present even if there are zero arguments.
...
[source]
----
fn concatenate(a:str, b:str):str = a + b
println concatenate("ab","ba")
...
[source]
----
fn concatenate(a:str, b:str):str = 13
try:
    println concatenate("ab","ba")
catch err: println err
...
[source]
----
Cannot return from 'concatenate' the value '13' as it does not fit any of the accepted type of the macro (str)
...
----
mut myString="num"
fn concatenate(a:str, b:str):(myString) = 13
myString = "str"
try:
    println concatenate("ab","ba")
catch err: println err
...
This is the identifier the function body uses for the variable as well as the identifier the caller uses if the parameter is passed by name.
...
Passing a value that does not fit the type declaration is an error.
...
You can name them any way you want, but for the sake of simplicity we will use the names above.
...
This will spread the content of the argument across the parameters.
...
* `LexList` the list of tokens.
Just like the class `Lex`, it is used in preprocessors.

* `Closure` is the implementation class for functions and closures.
It has one read-only field, `name`.
...
The following classes implement the `Iterable` interface in Turicum but do not implement `HasField`, and thus are spread as positional parameters:

* `IndexedString` envelops a string during commands that modify a String.
This is the technical implementation that internally replaces an immutable Java String with a mutable wrapper.
...
It makes it possible to use the result of an `async` expression as arguments.
+
...
Also note that global methods you want to use, like `str` in the example, have to be declared as `global`.
Without that the call will revert to the joker method itself.
...
When `fun` is uncurried in the last call, it gets disconnected from the object and is called as a standalone closure, without the surrounding object and class.
...
In the example above the class contains the method `init`, which is a special one, and declares one variable, `c`.

The method `init` is the initializer method.
...
If you use a variable without the `this.` prefix, it will be resolved automatically, but you can always use the `this.` prefix explicitly.
...
The variables `cls` and `this` are pinned and cannot be modified in methods.
...
When calling the constructor of the class to create a new instance, the variable `this` would be ambiguous.

For `a.Z()` it usually points to `a`, but if `Z` is a class and we called the constructor, then `this` should be the new instance.
...
[source]
----
this {that: {this: {...}, cls: class A}, this: {...}, cls: class B}
cls class B
that {this: {...}, cls: class A}
...
Classes support inheritance in Turicum.
...
You can use the methods or fields declared in the class, as well as any other methods and fields defined in the parent classes.
...
[source]
----
init in C, cls='class C' this='{this: {...}, cls: class C}'
...
----
init in C, cls='class C' this='{this: {...}, cls: class C}'
init in P1, cls='class P1' this='{this: {...}, cls: class C}'
init in P2, cls='class P2' this='{this: {...}, cls: class C}'
...
[source]
----
init in P, cls='class C' this='{this: {...}, cls: class C}'
...
There is no restriction on which other class initializers an initializer can invoke.
Your code could theoretically call the initializer of a class that the current class does not extend.
...
.output of snippet `veil_fields`
[source]
----
total: 150
caught: Field 'balance' is veiled.
caught: Field 'helper' is veiled.
front: internal
balance in keys: false
balance in keys_all: true
same: true
same: false
...
It will retain the exception's value during execution of the catch block.

The exception variable can be defined as `catch(e)` with parentheses to make it more familiar to Java developers.
Using the extra parentheses is not idiomatic in Turicum.
...
The exception variable is pinned and cannot be modified.
...
It is an error to execute a `return`, `break` or `continue` inside the `finally` block.

[[cleanup-after-halt]]
==== Cleanup After a Halt (Step Limit / Abort)

Two things can stop a Turicum program in a way that ordinary Turicum code cannot catch, suppress, or delay: the *step limit* being reached (see the `steps` option of `async`, above, and the embedder-configured interpreter-wide limit), and an *abort* requested from outside the program (for example, a supervising thread cancelling a task).
Both are enforced at the language boundary, deliberately bypassing `try`/`catch` entirely — a program running under a step limit must not be able to catch the limit and keep going, since that would defeat the whole point of using it as a sandboxing tool.

By default, this means a `finally` block (or a `with` block's `exit()` method) does not get a chance to do anything when one of these events happens: its very first statement hits the same limit and stops immediately, before it can, for example, close a file handle or release a lock.
The one exception is `mutex`/`sync`, whose release is implemented directly in the host language below the interpreter loop, and is therefore unaffected by any of this.

An embedder can opt in to a bounded *cleanup grace* period: a small, fixed number of extra steps that a `finally`/`exit()` block is allowed to spend releasing resources after a halt, before the halt becomes final.
This is configured on the Java side, not from Turicum source, since it is a property of how the interpreter is embedded rather than of the language itself:

[source,java]
----
// stepLimit:  the normal execution budget
// graceSteps: extra steps a finally/exit block may spend cleaning up after a halt fires
LocalContext ctx = new LocalContext(stepLimit, graceSteps);
----

With `graceSteps` left at its default of `0`, behavior is unchanged: cleanup blocks never run after a halt.
Configuring it grants a strictly bounded window, with three guarantees that keep it from reopening the hole the step limit exists to close:

* The grace budget is spent once, across the *whole* unwind — not once per `try`/`finally` or `with` block.
If a `finally` is nested inside another `finally`, the two share the same allowance; the outer one does not get a fresh window just because the inner one already used its share.

* Once the budget runs out, the halt becomes final and unconditional: no further Turicum code runs at all, including any other cleanup block still waiting its turn.
There is no way to be granted a second grace period.

* A `finally` or `exit()` cannot suppress a halt.
`exit()` returning `true` normally tells `with` to swallow an exception; during a halt this has no effect at all — the halt propagates regardless of what the cleanup code returns.

The effective ceiling on a program's execution is therefore `stepLimit + graceSteps`, fixed and known in advance — the same shape as the SIGTERM-then-SIGKILL pattern used by process supervisors: the halt fires, a bounded interval is granted for a clean shutdown, and if that is not enough, termination happens unconditionally.

== Decorators, Metaprogramming

Decorators are an inherent part of Turicum.
To make them similar to Python decorators and Java annotations, they can have a `@` in front of the decorator function call.
...
The code can, however, be fixed not only by using `@` but also by passing the function definition as a regular argument between `(` and `)`:
...
They can also modify an existing definition.
...
Note that if `fun_decorator` were a function declared with `fn`, it would not work.
...
The value is the closure or function itself, whatever we decorate.
...
The major difference between macros and closures is that the arguments of a closure are evaluated before the call.
...
You can use it to make recursive calls.
This is necessary because a function defined in a local context does not capture the surrounding context.
For more, read the next section.
...
[source]
----
println "abraka badraka".after("ra")
...
+
.source of snippet `string_after_last`
[source]
----
println "abraka badraka".after_last("ra")

----
+
.output of snippet `string_after_last`
[source]
----
ka
...
[source]
----
println "abraka badraka".before("ra")
...
+
.source of snippet `string_before_last`
[source]
----
println "abraka badraka".before_last("ra")

----
+
.output of snippet `string_before_last`
[source]
----
abraka bad
...
// is the same as
println "alma ata".after("al").before("al","ta")
...
+
.source of snippet `compile1`
[source]
----
mut z = "println 1+1".compile()
z.execute()

----
+
.output of snippet `compile1`
[source]
----
2
...
+
.source of snippet `compile2`
[source]
----
// the program contains one command, which is a block
mut z = "{let x = 1}".compile()[0] // get the block
z.execute()
print is_defined(x)

----
+
.output of snippet `compile2`
[source]
----
false
----
...
+
.source of snippet `compile3`
[source]
----
mut z = "{let x = 1}".compile()[0]
unwrap(z).execute()
print is_defined(x)," and it is ", x

----
+
.output of snippet `compile3`
[source]
----
true and it is 1
----
...
[source]
----
bin"626f72726120736f72206d656767796f746f72"
...
+
.source of snippet `string_from_hex`
[source]
----
println "48656c6c6f".from_hex()
println "0x48656C6C6F".from_hex().text()

----
+
.output of snippet `string_from_hex`
[source]
----
bin"48656c6c6f"
Hello
...
+
.source of snippet `string_bigi`
[source]
----
print "11111111111111111111111111111111111111111111111111111111111".big_int() +
      "11111111111111111111111111111111111111111111111111111111111".big_int()

----
+
.output of snippet `string_bigi`
[source]
----
...
+
.source of snippet `string_bigd`
[source]
----
print "0.11111111111111111111111111111111111111111111111111111111111".big_decimal() +
      "0.11111111111111111111111111111111111111111111111111111111111".big_decimal()

----
+
.output of snippet `string_bigd`
[source]
----
0.22222222222222222222222222222222222222222222222222222222222
----
...
+
.source of snippet `string_path`
[source]
----
println type("C:\\Users\\user\\Documents\\my_file.txt".path())

----
+
.output of snippet `string_path`
[source]
----
java.sun.nio.fs.UnixPath
...
+
.source of snippet `string_uri`
[source]
----
println type("http://cnn.com".uri())

----
+
.output of snippet `string_uri`
[source]
----
java.java.net.URI
...
+
.source of snippet `string_url`
[source]
----
println type("http://cnn.com".url())

----
+
.output of snippet `string_url`
[source]
----
java.java.net.URL
...
println "alma ata".bytes("Cp037") // EBCDIC encoding
...
[source]
----
bin"616c6d6120617461"
bin"819394814081a381"
...
+
.source of snippet `string_chars`
[source]
----
println jsonify("Hello, World!".chars())

----
+
.output of snippet `string_chars`
[source]
----
["H","e","l","l","o",","," ","W","o","r","l","d","!"]
...
== Bin Values and Functions

A `bin` value is a byte array.
It is the type Turicum uses for binary data: encoded text, hashes, base64 and hex decoded content, and binary file content.
You can create a `bin` value with the built-in function `bin()` (see the built-in functions), or from a string with the methods `bytes()`, `from_hex()`, and `from_base64()`.

The bytes of a `bin` are presented unsigned on the language surface.
Reading a byte — indexing, iteration, `to_list()` — gives an integer in the range `0..255`.
Writing a byte accepts an integer in the range `-128..255` and stores the low 8 bits; any other value is an error.

A `bin` is mutable and has a fixed length.
Assigning to a single index, as in `b[0] = 255`, modifies the byte array in place, and the change is visible through every variable that refers to the same `bin`.
Use the method `copy()` when you need an independent value.
Operations that change the length — concatenation with `+`, repetition with `*`, slicing — always create a new `bin`.
Assigning a `bin` to an index range, as in `b[1..3] = bin([9])`, may also change the length; in that case the variable is updated to hold a new byte array, and other variables that referred to the original `bin` keep the unchanged original.

You can index and slice a `bin` the same way as a string: `b[1]` is the byte at position one, `b[1..3]` is a new `bin` with the bytes of the range, and negative range ends count from the end of the value.
You can iterate over the bytes with `for each`.
The function `len()` gives the number of bytes.

The operators work on `bin` values the following way:

* `+` concatenates two `bin` values into a new one; mixing a `bin` with any other type on the right side of `+` is an error, and has to be converted explicitly.
* `*` with an integer repeats the content, like for strings.
* `==` and `!=` compare the content, byte by byte.
* `<`, `\<=`, `>`, `>=` order `bin` values lexicographically by the unsigned byte values.
* `===` is `true` only when the two operands are the same byte array.
* `in` tests containment: a byte value, as in `2 in b`, or a contiguous subsequence, as in `bin([1,2]) in b`.

A `bin` cannot be used as a boolean, and `jsonify()` refuses it; convert explicitly with `base64()`, `hex()`, or `to_list()`.

When a `bin` is converted to a string — printing, interpolation, `str()` — the result is `+bin"..."+` with the content in hexadecimal, for example `+bin"48656c6c6f"+`.
The hexadecimal content is the same string the method `hex()` returns and the string method `from_hex()` accepts.

The methods callable on `bin` values are the following:
...
The following example demonstrates the use of the `bin` methods and operators:

.source of snippet `bin_functions`

[source]
----
println " 1. ", bin([72, 101, 108, 108, 111])
println " 2. ", "Hello".bytes()
println " 3. ", "Hello".bytes().text()
println " 4. ", "Hello".bytes().hex()
println " 5. ", "48656c6c6f".from_hex()
println " 6. ", "Hello".bytes().base64()
println " 7. ", "SGVsbG8=".from_base64()
println " 8. ", "Hello".bytes().to_list()
println " 9. ", len("Hello".bytes())
println "10. ", "Hello".bytes()[1]
println "11. ", "Hello".bytes()[1..4]
println "12. ", bin([1, 2]) + bin([3])
println "13. ", bin([7]) * 3
println "14. ", "Hello".bytes().md5().hex()
println "15. ", bin([1, 2, 3, 2, 1]).index_of(2)
println "16. ", bin([1, 2, 3, 2, 1]).last_index_of(2)
println "17. ", bin([1, 2, 3]).contains(bin([2, 3]))
println "18. ", bin([1, 2, 3]).starts_with(bin([1]))
println "19. ", bin([1, 2, 3]).ends_with(bin([3]))
println "20. ", bin([1, 1, 1, 1]).count(bin([1, 1]))
println "21. ", bin([1, 2, 3]).left(2)
println "22. ", bin([1, 2, 3]).right(2)
println "23. ", bin([1, 2, 3]).reverse()
println "24. ", bin([255, 0]).xor(bin([15, 15]))
println "25. ", bin(4).fill(7)
println "26. ", bin(4).set(1, bin([9, 9]))
mut b = bin(8)
b.set_u32_at(0, 305419896)
println "27. ", b
println "28. ", b.u32_at(0)
println "29. ", b.u16_at(0, "le")
println "30. ", b.u32_at(0, "3412")
println "31. ", bin([128]).u8_at(0), " ", bin([128]).i8_at(0)
mut m = bin([1, 2, 3])
m[0] = 255
println "32. ", m
m[1..2] = bin([8, 8, 8])
println "33. ", m
println "34. ", 2 in bin([1, 2, 3])
for each x in bin([10, 20]) : println "35. ", x

----

.output of snippet `bin_functions`
[source]
----
 1. bin"48656c6c6f"
 2. bin"48656c6c6f"
...
12. bin"010203"
13. bin"070707"
14. 8b1a9953c4611296a827abf8c47804d7
15. 1
16. 3
17. true
18. true
...
Turicum implements many mathematical functions.
All mathematical functions from Java's Math class are available in Turicum.
In addition to that, there are some Turicum-specific ones also.
...
----
let z=thunk( { let k= 5 } )
println jsonify_beauty( as_object( z ) )
    ...
    "commands": [
                    {
                        ...
                        "java$canonicalName": "ch.turic.commands.LetAssignment",
                        "mut": false
                    }
                ]
}
...
for each command in commands {
    ...
    if i == 1 {// k is not defined, no assignable
        try: unthunk(command) catch e: println "ERROR: %s" % [e];
    } else {
        // 2 can assign value to 'suss'
        // 3 can define 'x'
        // 4 prints 'hello'
        unthunk(command)
...
.source of snippet `bin`

[source]
----
println bin()
println bin(3)
println bin([65, 66, 67])
println bin("túró")
println bin("túró", "ISO-8859-2")
println bin(bin([1, 2, 3]))

----

.output of snippet `bin`
[source]
----
bin""
bin"000000"
...
.source of snippet `curried_arity`

[source]
----
fn add(a,b)=a + b
let add5 = add.(5)
fn test(s){ println s," = ", s.execute() }
test "add5(5)"
test "curried_arity(add5)"
test "arity(add5)"
test "curried_arity(add)"
test "arity(add)"

----

.output of snippet `curried_arity`
[source]
----
add5(5) = 10
curried_arity(add5) = 1
arity(add5) = 2
curried_arity(add) = 0
arity(add) = 2
...
[source]
----
let a = { a: 1 , b: 2, c: ["kedavra"], d:"avraka" , h: none}
a.a = none // this does not delete the field
println "'a' set to none ", jsonify(a)
delete a, a // delete the field a
println "deleted field 'a' ", jsonify(a)
delete a.h
println "deleted field 'h' via field access", jsonify(a)
delete {13; 55; a }, b // the object is calculated
delete a, "c" // string name for the field
println "deleted field 'c' using a string ", jsonify(a)
delete a, chr("c".ord()+1) // string calculated name
println "deleted field 'd' using calculated string ", a
...
[source]
----
'a' set to none {"a":null,"b":2,"c":["kedavra"],"d":"avraka","h":null}
deleted field 'a' {"b":2,"c":["kedavra"],"d":"avraka","h":null}
deleted field 'h' via field access{"b":2,"c":["kedavra"],"d":"avraka"}
deleted field 'c' using a string {"d":"avraka"}
deleted field 'd' using calculated string {}
...
println jsonify_beauty(glob("**/test-classes/**/*.class", recursive=true)[0..3])
...
[
    ...
    "target/test-classes/ch/turic/TestTuriParameters$AnnotatedBuiltin.class"
]
...
println is_curried(add)
println is_curried(add5)
println is_curried(unc)

----

.output of snippet `is_curried`
[source]
----
false
true
...
.source of snippet `is_obj`

[source]
----
fn test(s){println jsonify(s), "=" , "is object" if is_obj(s) else "is not object"}
...
mut K = class : B {}
mut z = K()
println is_type(z,"A"),""" "A" yes, it is the root class"""
println is_type(z,A)," A yes, when referencing the class itself"
println is_type(z,"H"),""" "H" no, there is not even defined"""
try: is_type(z,H) catch e: println $"H is not defined: ${e}"
println is_type(z,"K"),""" "K" no, it is not the name of the klass, only a variable"""
println is_type(z,K)," K yes, it is the class itself"
println is_type(z,"#undefined")," #ndefined yes, it is also has a class type that is unnamed"
...
[source]
----
true "A" yes, it is the root class
true A yes, when referencing the class itself
false "H" no, there is not even defined
H is not defined: Variable 'H' is undefined.
false "K" no, it is not the name of the klass, only a variable
true K yes, it is the class itself
true #ndefined yes, it is also has a class type that is unnamed
...
java_call sb,"deleteCharAt",java_int(0)
println sb
...
.source of snippet `java_import`

[source]
----
println jsonify_beauty(java_import("ch.turic.exceptions"))

----

.output of snippet `java_import`
[source]
----
{
    "ExecutionAborted": JavaClass[klass=class ch.turic.exceptions.ExecutionAborted],
    "ch.turic.exceptions.StringNotTerminated": JavaClass[klass=class ch.turic.exceptions.StringNotTerminated],
    "ch.turic.exceptions.StepLimitReached": JavaClass[klass=class ch.turic.exceptions.StepLimitReached],
    "InterpreterHalt": JavaClass[klass=class ch.turic.exceptions.InterpreterHalt],
    "ch.turic.exceptions.UndefinedVariable": JavaClass[klass=class ch.turic.exceptions.UndefinedVariable],
    "BadSyntax": JavaClass[klass=class ch.turic.exceptions.BadSyntax],
    "ch.turic.exceptions.ExecutionException": JavaClass[klass=class ch.turic.exceptions.ExecutionException],
    "ch.turic.exceptions.UnexpectedCharacter": JavaClass[klass=class ch.turic.exceptions.UnexpectedCharacter],
    "ch.turic.exceptions.InterpreterHalt": JavaClass[klass=class ch.turic.exceptions.InterpreterHalt],
    "UnexpectedCharacter": JavaClass[klass=class ch.turic.exceptions.UnexpectedCharacter],
    "StepLimitReached": JavaClass[klass=class ch.turic.exceptions.StepLimitReached],
    "StringNotTerminated": JavaClass[klass=class ch.turic.exceptions.StringNotTerminated],
    "ch.turic.exceptions.BadSyntax": JavaClass[klass=class ch.turic.exceptions.BadSyntax],
    "UndefinedVariable": JavaClass[klass=class ch.turic.exceptions.UndefinedVariable],
    "ExecutionException": JavaClass[klass=class ch.turic.exceptions.ExecutionException],
    "ch.turic.exceptions.ExecutionAborted": JavaClass[klass=class ch.turic.exceptions.ExecutionAborted]
}
...
.source of snippet `java_callback1`

[source]
----
// note you can use the keyword 'as' without the backticks
println java_callback(fn(x) = x*2).as("IntFunction").apply(java_int(2))

----

.output of snippet `java_callback1`
[source]
----
4
...
.source of snippet `java_resources`

[source]
----
let appia_handlers = java_resources("turi/appia.turi")
println $"There is ${len(appia_handlers)} appia handler sys import file."
println $"It contains ${len(appia_handlers[0])} characters."

----

.output of snippet `java_resources`
[source]
----
There is 1 appia handler sys import file.
It contains 756 characters.
...
.source of snippet `java_type`

[source]
----
println java_type(42), " ", type(42)
println java_type(java_int(42)), " ", type(java_int(42))
println java_type(thunk({let z=2}))," ",type(thunk({let z=2}))
println java_type(java_class("java.math.BigInteger"))," ",type(java_class("java.math.BigInteger"))
println java_type(java_class("java.math.BigInteger")("42"))," ",type(java_class("java.math.BigInteger")("42"))

----

.output of snippet `java_type`
[source]
----
java.lang.Long int
java.lang.Integer num
ch.turic.commands.BlockCommand java.ch.turic.commands.BlockCommand
ch.turic.memory.JavaClass fn
...
let h = a()
println $" $(h.h) $(h.i) $(h.j) $(h.k) $(h.l) $(h.m) $(h.n) $(h.o) $(h.p) $(h.q) $(h.s)"

----

.output of snippet `pack_1`
[source]
----
...
println $"y.h = $(y.h)"
x.h = 100
println "AFTER x.h = 100"
println $"x.h = $(x.h)"
println $"y.h = $(y.h)"
println $"are they the same? $(x === y)"

// field w is defined anew, not shared, inserted into x and y separately
x.w = "x.w"
y.w = "y.w"
println $"x.w = $(x.w)"
println $"y.w = $(y.w)"
...
// try_receive() returns none when there is nothing in the queue
println q.try_receive()," return none, there is nothing in the queue"

// it also returns none when there is a none in the queue
q.send(none)
...
fn printer(q,@name){
    ...
    while {
        ...
        return { println "%s is done" % name } when q.is_closed(); // return when the que is closed
        if s == none {
...
task2 is not ready 1task1 is not ready 1
...
task2 is not ready 6task1 is not ready 2
...
task2 is not ready 2
task2 is not ready 3task1 is ready 4
...
task2 is ready 5
task2 is not ready 0
task2 is not ready 1task1 is not ready 5
...
task1 is not ready 9task2 is not ready 3
...
The macro `quote` is an alias for the macro `thunk`.
The name is a complimentary addition to honour the functionality from LISP.
...
The `*_force` version overwrites the value even when the variable is pinned.
...
.source of snippet `unwrap1`

[source]
----
let a = 1; // defined 'a' outside

// wrapping block
{
    let a = 12; // redefining 'a'  for the local use
    let b = 77; // defining 'b' valid only  inside
}

println "AFTER THE WRAPPING BLOCK"
println $"a is $a"
println $"b is not defined is_defined(b) is ${is_defined(b)}"

unwrap(thunk({
    try: let a = 12; // redefining 'a' will raise error
    catch e: println "redefinition is not possible non-wrapped"
    let b = 77; // defining 'b' valid outside when unwrapped
})).execute()

println "AFTER THE UNWRAPPING BLOCK"
println $"a is $a"
println $"b is not defined is_defined(b) is ${is_defined(b)}"
println $"b is $b"

// you can get the same result using an expression with a non-wrapping part
!(
    try: let a = 12; // redefining 'a'  will raise error
    catch e1: println "redefinition is not possible non-wrapped"
    let k = 78; // defining 'k' valid also outside
)

println "AFTER THE UNWRAPPING EXPRESSION"
println $"a is $a"
println $"k is $k"
...
.output of snippet `unwrap1`
[source]
----
AFTER THE WRAPPING BLOCK
a is 1
b is not defined is_defined(b) is false
redefinition is not possible non-wrapped
AFTER THE UNWRAPPING BLOCK
a is 1
b is not defined is_defined(b) is true
b is 77
redefinition is not possible non-wrapped
AFTER THE UNWRAPPING EXPRESSION
a is 1
k is 78
...
.source of snippet `xml_format`

[source]
----
println xml_format("""<a>b<marnacle xmlns="hupsi"></marnacle> </a>""")

try xml_format("<a>b<") catch e: println e

----

.output of snippet `xml_format`
[source]
----
<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<a>
    b
    <marnacle xmlns="hupsi"/>
</a>

There was an XML exception formatting XML
...
fn printer(){
    ...
    while {
        mut s = try_yield();
        return { println "we are done" } when yield_is_closed();
        if s == none {
...
You can also use the macro `quote`, which is an alias and does the same as `thunk`, or use the `'` (single apostrophe) operator.
This deferred computation can later be executed using the function `unthunk` or the `execute` method defined on the resulting command object.

`unthunk` and `execute` evaluate the stored command in the **context where `unthunk` is called**, not the context where the thunk was created.
...
- `evaluate` can only be used **inside a macro**, or in a function or closure.

- It evaluates the argument in the **caller’s context**, i.e., the context of the macro's caller.
====

Note also that the argument to `thunk`, `quote`, or the operator `pass:[']` can only be an expression and not an arbitrary command.
That is a restriction imposed by Turicum syntax.
However, you can enclose any command in a block, and then you can use the block as an expression or part of an expression.

==== `unthunk`

When you `unthunk` a block it will be evaluated in a local context freshly created for the block execution.
Although this context wraps the surrounding context, locally defined variables will not be visible after the execution of the block.
...
`unwrap` takes a block command as its argument, typically the result of a `thunk` call, and returns a non-wrapping copy of the block.
====
...
The syntax analyzer works on the token list.
When it encounters a `#` symbol, it starts preprocessing for the remainder of the token list.

The steps of the preprocessing are the following:

. First of all, it parses the command following the `#` symbol.
It has to be a macro, a function, or a closure.
This is the preprocessor command.

. After that, it removes all tokens from the processed token list.
This includes the tokens just parsed after the `#` character.

. The parser creates an interpreter context to execute the preprocessor command.
It is the preprocessor context.
This context is created and reused by the analyzer if multiple preprocessors are executed one after the other.

. The analyzer executes the commands just parsed using the preprocessor context.
This will create global variables, functions, and classes.

. It executes the command, passing the list of tokens as the only argument.
That is why the command has to be a macro, a function, or a closure.
The command should read the token list and result in a list.
This list may contain the original tokens, newly created tokens, and strings.

. It removes all the tokens from the input and replaces them with the token list returned by the preprocessor.

. If there are non-token-list elements, it parses them and converts each into a list of tokens.

The analyzer uses this new token list to analyze the program code.
The commands created up to this point are erased from the program being built.
They served only the purpose of the preprocessor.

While the remaining code is analyzed, the interpreter will use the already existing preprocessor context in subsequent preprocessing steps.

The following example shows preprocessing that adds lines defining
...
A newline can replace the use of `;`.
...
----
   ...
   LOOP ::= ( FOR | FOR_EACH ) [ 'until' EXPRESSION];
...
Pinning has no effect on outer scopes.
...
It is valid to have no identifiers in an identifier list, i.e., an empty identifier list.
...
Field access: obj.field
 ...
 FIELD_ACCESS: := PRIMARY_EXPRESSION ('.'|'?.') (IDENTIFIER | KEYWORD) ;

Method call: obj.method(params)

 METHOD_CALL: := PRIMARY_EXPRESSION ('.'|'?.') (IDENTIFIER | KEYWORD) '(' [ EXPRESSION { ',' EXPRESSION } ] ')' ;
