...
The `with` command is used to execute a block in the context of one or more object.
...
When more than one objects are listed then multiple wrapping scopes are created from left to right.
...
When an object has the method `entry` it is invoked before the body of the `with` command is executed.
When an object has the method `exit` it is invoked after the body of the `with` command is executed.


.source of snippet `entryAndExitWith`
[source]
----
class A {
    fn init(a=1,b=2);
...
.output of snippet `entryAndExitWith`
[source]
----
entry to A
entry to B
executing with A and B
exit fom B
exit fom A
...
It is also possible to replace the object in the method `entry`.
The method `entry` can return `none` signalling that the original object has to be used, or
it can return an arbitrary object.
If the method `entry` returns a different object that will be used instead of the one listed in the `with` command.
The `exit` method will still be invoked on the original object and not the one returned by `entry`.


.source of snippet `entryAndExitWith2`
[source]
----
class A {
    fn init(a=1,b=2);
...
.output of snippet `entryAndExitWith2`
[source]
----
entry to A
entry to B
...
This working is very similar how Python handles the `with` command.
...
[source]
----
ch.turic.commands.Closure@1caa0244
2
...
[source]
----
ch.turic.commands.Closure@7ce026d3
2
...
[source]
----
ch.turic.commands.Closure@7ce69770
2
...
[source]
----
ch.turic.commands.Closure@1750fbeb
6
...
task1 is not ready 3task2 is ready 4
...
task2 is not ready 3
task2 is not ready 4task1 is ready 6
