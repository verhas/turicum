...
When you do that, it is essentially the same as assigning a new value to the variable altogether:
...
The following example demonstrates that `otherwise` is not executed when the loop is executed at lead once.
...
In Python, it is executed when the loop is broken.
...
It also a normal termination of the loop when the loop is terminated by the `until` condition:
...
The `with` command can handle classes similar to object and also as resource managers.
...
* `time` limits the time of the task, unit is seconds.
The task will stop after the specified time.
When a task times out the underlying thread stops and the task `is_done()`, `is_err()`, and `is_timeout()` methods will return true

These parameters can be specified after the `async` keyword between `[` and `]` characters.
When the command to be executed asynchronously starts with a `[` character you either has to have `[]` in front of it,
...
[NOTE]
====
You should not enclose the list literal between `(` and `)` using async.
...
It is necessary that when a function is defined in a local context, it does not capture.
...
When it meets a `#` symbol, it starts preprocessing for the remainder of the token list.
...
That is why the command has to be a macro, a function of closure.
...
The analyzer uses this new token list to analyse the program code.
