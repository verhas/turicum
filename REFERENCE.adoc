...
The `flow` command defines a *reactive execution block* in which named expressions (called *cells*) are evaluated in a concurrent and data-driven fashion. It is particularly useful for implementing iterative or mutually dependent computations where values must be recalculated until a condition is satisfied, or a fixed number of changes have occurred.

Cells are defined as variable-expression bindings, and their dependencies are automatically detected by analyzing identifier usage. When the value of a cell changes, all other cells that depend on it are automatically rescheduled for evaluation in a new thread (using Java virtual threads).
...
If a `yield` expression is specified, its result is returned as the result of the `flow` command. Otherwise, `null` is returned.
...
[source]
----
flow [until <condition>] [limit <maxTasks>] [timeout <seconds>] {
...
- `until <condition>`::
Optional. A boolean expression that terminates the flow once it evaluates to true.
- `limit <maxTasks>`::
Optional. A numeric constant that sets the maximum number of dependent cell executions allowed.
- `timeout <seconds>`::
Optional. A numeric constant (may be fractional) specifying the maximum wall-clock time allowed for execution, in seconds.
- `yield <expression>`::
Optional. Defines the return value of the `flow`. It must be the last statement in the block.

=== Semantics

The `flow` block contains a series of *cells*. Each cell defines a named variable and an expression that computes its value. These variables can reference each other, and the runtime will detect and track these dependencies.

When a cell is updated (i.e., its computed value differs from the previous value in the local context), all cells that depend on it are automatically rescheduled. This process continues until one of the termination conditions is met.

Cell evaluation is performed in isolated thread-local contexts derived from the flow's original context. Variables are copied and frozen in each thread.
...
x =5
x =6
Task limit has been reached in flow command after 5 tasks.
Final x = none
...
- The order of `until`, `limit`, and `timeout` is flexible.
...
[source]
----
ch.turic.commands.Closure@231f98ef
2
...
[source]
----
ch.turic.commands.Closure@7c137fd5
2
...
[source]
----
ch.turic.commands.Closure@183ec003
2
...
[source]
----
ch.turic.commands.Closure@7d9d0818
6
...
task2 is not ready 14task1 is not ready 2

task2 is not ready 15task1 is not ready 3
