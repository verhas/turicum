...
* Any string between  (`pass:[$`]`) and  (`pass:[`]`) is treated as an interpolated identifier.
...
You can specify interpolated strings as identifiers using  (`pass:[$`]`) and  (`pass:[`]`).
To use interpolated identifiers, the same rules apply as in the case of interpolated strings.
The exception is that you do not need to escape the `pass:["]` characters, but must escape the `pass:[`]` characters, for obvious reason.

You are not allowed to use interpolated identifiers to

* define a cell for a `flow` command, and

* define the `init` method of a class if the method does not have a body.

[WARN]
====
The interpolated identifier feature is a very powerful meta-programming tool.
It is very easy to create overcomplicated, non-maintainable, and unreadable code.
Use it with caution and only as a last resort.
====
...
If you use the format `let { identifier list } = expression` or `mut { identifier list } = expression` then the identifiers are not followed by expressions individually.
...
[source]
----
Assigned value changed while calculating new value z[ch.turic.commands.StringConstant@7829b776]
12 already changed
----
...
If the function name, or any other identifier (parameter names, type names) are interpolated, then they will be evaluated when the function definition is executed.

.source of snippet `fun13`
[source]
----
let zz = "alma"
mut b = "c"
...
In the example above both the function name and the second parameter are interpolated.
This is also an example how to use `pass:[`]` defined identifiers in the interpolated expressions inside an interpolated identifier.
The name of the function and the name of the parameter are evaluated at the time the function definition is executed.
It does not matter that the global variable `b`, referenced in the interpolated identifier in the second parameter is modified and has a different value at the time of the function call.
...
It is possible to define a special method in a class or attached to an object, named `pass[.]` (a single dot), which is automatically be invoked when the code tries to call a method not defined in the object or class hierarchy.
...
[source]
----
[Lex{types=RESERVED, text='for'}, Lex{types=RESERVED, text='each'}, Lex{types=IDENTIFIER, text='i'}, Lex{types=RESERVED, text='in'}, Lex{types=IDENTIFIER, text='range'}, Lex{types=RESERVED, text='('}, Lex{types=INTEGER, text='1'}, Lex{types=RESERVED, text=','}, Lex{types=INTEGER, text='55'}, Lex{types=RESERVED, text=')'}, Lex{types=RESERVED, text=':'}, Lex{types=RESERVED, text='println'}, Lex{types=IDENTIFIER, text='i'}]
...
[source]
----
{java$canonicalName: ch.turic.commands.BlockCommand, wrap: true, commands: [{assignments: [{identifier: {name: k, java$canonicalName: ch.turic.commands.Identifier, commands: []}, types: [], expression: {java$canonicalName: ch.turic.commands.IntegerConstant, value: 5}, java$canonicalName: ch.turic.analyzer.AssignmentList.Assignment}], java$canonicalName: ch.turic.commands.LetAssignment, mut: false}]}
