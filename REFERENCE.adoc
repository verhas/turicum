...
Evaluate the function argument as a command in the caller environment.
If you want to evaluate a string containing Turicum code, you have to use `compile()`.
...
[NOTE]
====
The character `|` was selected because it works on Windows as well as on Linux or other Unix-based operating systems.
...
You can assign the returned object to a variable or use it right away, cherry-picking certain fields from it.

If you want a specific class or function in your namespace, you can simply `let` assign it to a variable.
...
If the local context changes, the object changes accordingly.
...
Changing a field in one will change the field in the other, even though the two objects are not the same.
...
Through the returned object, you can access the closure's wrapped context, in this case, the variable `b`, even though this is not a field in the object.
...
[
    ...
    "target/test-classes/ch/turic/AdHoc.class",
    ...
    "target/test-classes/ch/turic/TestBasic.class",
...
task1 is ready 3task2 is not ready 3
...
You can set the name as a global name, but that will define the name globally.
If the function is in a local context, it may not exactly work as expected.

Calling this function from inside another function can redefine a variable in the context of the code that calls the function or the closure.
The `*_force` version overwrites the value even when the variable is pinned.
...
The macro should have exactly one or two arguments.
...
That way you can delete the field named `b` of object `a` as
