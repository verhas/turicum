...
The operator `<<` can also attach an object to a list.
The left-hand side operand has to be a list, the right-hand side operand is an object or a class.
The result is a special list that references all the fields of the right-hand side object or class.
If the field is a function, closure or macro then the usual special fields: `this` and `cls` are set the same way as if the call was performed on the original object or class.
In addition to that the special field `it` will point to the list.

The result list will be a copy, but it will reference the object or class.
The object or class is not copied.
If the object or class changes, this change will reflect through the list as well.
...
[source]
----
Assigned value changed while calculating new value z[ch.turic.commands.StringConstant@645aa696]
12 already changed
----
...
3
Thread 1 generated  prime read 5
5
...
println((""+inc).left(len("ch.turic.commands.Closure@")))
println(inc(1))
...
task2 is not ready 1
task1 is not ready 8task2 is ready 4
...
task1 is not ready 2
task1 is ready 9task2 is not ready 10
